import {ElectronEncoder, type ParitySetting} from '../encoder/ElectronEncoder.js';
...
const definition: AdapterDefinition = {
  ...
  encode(recorder: RecorderInterface, ba: BufferAccess, options: OptionContainer) {
    ...
    while (chunkOffset < uefBa.length()) {
      ...
      interpretChunk(e, chunkType, chunkBa);
...
function interpretChunk(e: ElectronEncoder, chunkType: number, chunkBa: BufferAccess): void {
  switch (chunkType) {
    ...
    case 0x0104: // defined tape format data block
    {
      ...
      for (let i = 0; i < dataBa.length(); i++) {
        e.recordByte(dataBa.getUint8(i), dataBits, parity, stopBits); // TODO: check this stuff
      }
      break;
    }
    ...
    case 0x0111: // carrier tone with dummy byte
    {
      ...
      e.recordCarrier(cyclesBefore);
      e.recordByte(0xaa); // dummy byte
      e.recordCarrier(cyclesAfter);
      break;
    }
