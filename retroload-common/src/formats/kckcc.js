...
export class Adapter extends AbstractAdapter {
  ...
  static encode(recorder, dataView, options) {
    const blocks = Math.ceil(dataView.byteLength / fileBlockSize);

    if (blocks > 255) {
      throw new Error('KCC file contains too many blocks.');
    }
    if (dataView.byteLength % fileBlockSize !== 0) {
    ...
    for (let i = 0; i < blocks; i++) {
      ...
      const blockData = dataView.referencedSlice(fileOffset, fileBlockSize);
